...
vi.mock("../src/lib/url-builder.js", () => ({
  buildNotificationUrl: vi.fn(
    (notif) => `https://github.com/${notif.repository?.full_name || "test/repo"}`,
  ),
}));
...
describe("retry logic", () => {
  ...
  it.each([401, 403, 404])("should NOT retry on %i errors (non-retryable)", async (status) => {
    ...
    await expect(github.getNotifications()).rejects.toThrow(
...
describe("pollForToken expiresIn handling", () => {
  ...
  it("should use 15 minutes default when expiresIn is not provided", async () => {
    ...
    const pollPromise = github.pollForToken(
      "device_code_123",
      5 /* interval */,
      null /* expiresIn */,
    );
...
describe("Device Flow error handling", () => {
  ...
  it("should throw immediately for terminal errors like access_denied (no retry)", async () => {
    mockFetch.mockResolvedValue({
      ...
      status: 200,
      json: () =>
        Promise.resolve({ error: "access_denied", error_description: "Access was denied" }),
    });
