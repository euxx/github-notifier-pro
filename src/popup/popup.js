...
async function markAllAsRead() {
  ...
  try {
    ...
    if (result.success) {
      ...
      markAllBtn.disabled = true;
      markAllBtn.replaceChildren(...originalNodes.map((n) => n.cloneNode(true)));
      filter.clearFilteredAfterMarkAll();
    } else {
      rollback();
...
async function refresh() {
  ...
  try {
    ...
    filter.renderWithFiltered(state.notifications, true); // Re-sort on refresh
  } catch (error) {
    console.error("Failed to refresh:", error);

    // Show appropriate error message based on error type
    const cachedNotifications = await storage.getNotifications();
    filter.renderWithFiltered(cachedNotifications, true); // Re-sort even on error
...
async function login(authMethod = "oauth", token = null) {
  ...
  if (result.success) {
    ...
    const state = await sendMessage(MESSAGE_TYPES.GET_STATE);
    filter.renderWithFiltered(state.notifications, true); // Then render notifications
...
async function handleMarkRepoAsRead(repoFullName) {
  ...
  try {
    ...
    if (response.success) {
      ...
      clearNotificationCache();
      filter.renderWithFiltered(nextNotifications, false);
      if (filter.isShowingFiltered()) filter.renderFilteredInFilterView();
    } else {
      anim.rollback();
...
async function init() {
  ...
  if (state.isAuthenticated) {
    ...
    filter.renderWithFiltered(state.notifications, true); // Re-sort on init
...
browserStorage.onChanged.addListener((changes, areaName) => {
  if (areaName === "local" && changes.notifications && !mainView.hidden) {
    ...
    filter.renderWithFiltered(newNotifications, false);
    if (filter.isShowingFiltered()) filter.renderFilteredInFilterView();
  }

  // Filter view stats live update is owned by the filter module.
  filter.handleStorageChange(changes, areaName);
});
